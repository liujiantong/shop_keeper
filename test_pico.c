...
void process_image(IplImage* frame, int draw, int print)
{
	...
	*/
	ndetections = find_objects_rot(0.0f, rs, cs, ss, qs, MAXNDETECTIONS, appfinder, pixels, nrows, ncols, ldim, SCALEFACTOR, STRIDEFACTOR, minsize, MIN(nrows, ncols), 1);

#endif

	printf("found %d ndetections\n", ndetections);
	float rsf[ndetections], csf[ndetections], ssf[ndetections], qsf[ndetections];
	int counter = 0;
	if (ndetections == 1)
	{
		counter = 1;
		rsf[0] = rs[0]; csf[0] = cs[0]; ssf[0] = ss[0]; qsf[0] = qs[0];
	}
	else if(ndetections > 1)
	{
		rsf[0] = rs[0]; csf[0] = cs[0]; ssf[0] = ss[0];
		counter = 1;
		for (i=1; i<ndetections; ++i)
		{
			printf("ratio=%f\n", (fabs(rs[i]-rs[i-1]) + fabs(cs[i]-cs[i-1]))/ss[i] );
			if ( (fabs(rs[i]-rs[i-1]) + fabs(cs[i]-cs[i-1]) )/ss[i] > OVERLAPRATIO )
			{
				rsf[counter] = rs[i]; csf[counter] = cs[i]; ssf[counter] = ss[i], qsf[counter] = qs[i];
				counter ++;
			}
		}
	}
	...
	if(draw)
		...
		for(i=0; i<counter; ++i)
			if(qsf[i]>=QCUTOFF) // check the confidence threshold
				cvCircle(frame, cvPoint(csf[i], rsf[i]), ssf[i]/2, CV_RGB(255, 0, 0), 4, 8, 0); // we draw circles here since height-to-width ratio of the detected face regions is 1.0f
	...
	if(print)
	{
		...
		for(i=0; i<counter; ++i)
			if(qsf[i]>=QCUTOFF) // check the confidence threshold
				printf("%d %d %d %f\n", (int)rsf[i], (int)csf[i], (int)ssf[i], qsf[i]);
	}
}
