...
#define ORIENT_LEVEL 6
	int find_objects_rot(float ccwangle, float rs[], float cs[], float ss[], float qs[], int maxndetections,
						void* od,
						void* pixels, int nrows, int ncols, int ldim,
						float scalefactor, float stridefactor, float minsize, float maxsize,
						int clusterdetections)
	{
		int ndetections=0, i=0;
		float step_angle = 3.14f / ORIENT_LEVEL;
		for (i=0; i<3; ++i) {
			float orientation = (i-1) * step_angle;
			ndetections += find_objects(orientation, &rs[ndetections], &cs[ndetections], &ss[ndetections], \
				&qs[ndetections], maxndetections-ndetections, od, pixels, nrows, ncols, ldim, \
				scalefactor, stridefactor, minsize, maxsize, clusterdetections);
		}
